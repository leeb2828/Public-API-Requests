...
function locate_employee_object(name, num) {
    ...
    if (num === 0) {
        const obj = all_employees.filter(function(employee) {
            if (employee.name.first === name_array[0]) {
                return employee;
            } 
        });
        return obj;
    ...
    // This is for when the user clicks the "NEXT" button on the modal window
        ...
        for (let i = 0; i < all_employees.length; i++) {
            if (all_employees[i].name.first === name_array[0]) {
...
function add_information_to_modal_window(human) {
    ...
    <p class="modal-text cap">${person.location.city}, ${person.location.state} ${person.location.postcode}</p>
...
function hide_or_display_employees(typed_name) {
    ...
    for (let i = 0; i < people.length; i++) {
        ...
        if (name.textContent.toLowerCase().includes(typed_name.toLowerCase()) ) {
            item.style.display = "";
            found_length += 1;
        } else {
    ...
    console.log("hidden_length: ", hidden_length)
